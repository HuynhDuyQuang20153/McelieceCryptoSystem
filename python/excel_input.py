...
class Excel_input:
    def __init__(self, choose_type_file, already_assigned, encoder, t, k, n, orther, private_key, direc_cipherText, data_line):
        self.t = t
        self.k = k
        self.n = n
    ...
    def Excel_Write_input(self):
        ...
        new_dir_path = os.path.join(dir_path, "Save_turn_" + str(max_index + 1))
        if not os.path.exists(new_dir_path):
            os.mkdir(new_dir_path)

        path_cipher = os.path.join(new_dir_path, "CipherText")
        if not os.path.exists(path_cipher):
            os.mkdir(path_cipher)

        file_encoder_name = "encoder.joblib"
        encoder_path = os.path.join(path_cipher, file_encoder_name)
        joblib.dump(self.encoder, encoder_path)
        print(f"\t- Save encoder object    in: {encoder_path}")
        ...
        if self.choose_type_file == '1':
            input_file = os.path.join(to_path, "text_2.xlsx")
            output_file = os.path.join(to_path, "Save_file", new_dir_path, path_cipher, "Input.xlsx")
            shutil.copyfile(input_file, output_file)

            workbook_ex = openpyxl.Workbook()
            sheet_cipher = workbook_ex.active
            sheet_cipher.title = "cipher"
            file_name_cipher = "Cipher.xlsx"
            ...
            to_path = os.path.join(os.getcwd(), "..", "data", "Save_file", new_dir_path, path_cipher)

            input_file = os.path.join(os.getcwd(), "..", "data", "test.txt")
            output_file = os.path.join(to_path, 'Input.txt')
            shutil.copyfile(input_file, output_file)
